...
public class MessengerAspect {
	...
	public void checkGreeting(ProceedingJoinPoint joinPoint) throws Throwable{
		...
		for(Object tmp:args) {
			...
			if(tmp instanceof String) {
				...
				if(msg.contains("똥개")) {
					System.out.println("똥개는 금지된 언어 입니다!");
					return; // 메소드를 끝내기
				}
			}
		}
		...
		// proceed() 를 호출해야 aspect가 적용된 메소드가 실행된다.
		// joinPoint 객체는 around 어노테이션에서만 사용할 수 있다.
		joinPoint.proceed();
		
		// aspect가 적용된 메소드가 호출 되기 직전에 할 작업은 proceed() 호출 이후에 한다.
		
		System.out.println("aspect가 적용돼 메소드가 리턴 했습니다!");
	}
	
	/*
	 * 	return type은 String 이고
	 * 	get 으로 시작은 메소드 이고
	 * 	메소드에 전달되는 인자는 없다. 
	...
	public Object checkReturn(ProceedingJoinPoint joinPoint) throws Throwable{
		...
		// 원래 type으로 casting 해서 조사해 볼 수 가 있다.
		String a = (String)obj;
		
		// 조사 후 아예 다른 값을 리턴해 줄 수 도 있다.
		return "놀자 놀자";
	}
}
